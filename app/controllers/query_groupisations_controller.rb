class QueryGroupisationsController < ApplicationController
  ...
  def index
    @auctions = QueryGroupisation.all
  end

  def create

    @query = QueryGroupisation.new(query_params)
  ...
  def show
    ...
    @query= QueryGroupisation.where("intent like ?", params[:intent]).select("link").distinct
